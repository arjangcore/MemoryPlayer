...
{
    /// <summary>
    /// ColourWheel class used to determine which colours to automatically generate.
    /// Uses a hue-subdivision to select contrasting colours
    /// </summary>
    public class ColourWheel
    {

        private static ColourWheel mInstance = null;
        private Hashtable mColours;        

        ColourWheel()
        {
            mColours = new Hashtable();
            mColours[0]  = System.Drawing.Color.Red;
        }

        static private ColourWheel Instance()
        {
            if ( mInstance == null )
            {
                mInstance = new ColourWheel();
            }
        ...
        {            
            if ( Instance().mColours.ContainsKey( n ) )
            {
                return (Color)Instance().mColours[ n ];
            }

            return (Color)Instance().GenerateColour( n );
        }

        private Color GenerateColour( int n )
        {
            // Sort colours by Hue
            ArrayList sorted = new ArrayList();
            foreach ( Color c in mColours.Values )
            {                
                sorted.Add( c.GetHue() );
            }
            // Manually add the upperbound (since 360 == 0 in HSL space)
            sorted.Add( 360.0f );
            sorted.Sort();
            
            // Find max range, create new colour there.
            int maxRangeIndex = 0;
            float maxRangeValue = 0;
            for ( int i = 0; i < sorted.Count - 1; i++ )
            {
                float h1 = (float)sorted[ i ];
                float h2 = (float)sorted[ i + 1 ];                

                if ( maxRangeValue < h2 - h1 )
                {
                    maxRangeValue = h2 - h1;
                    maxRangeIndex = i;
                }
            ...
            float newHue = (float)sorted[ maxRangeIndex ] + (maxRangeValue / 2);
            return (Color)(mColours[ n ] = HSL_to_RGB( new HSL( newHue/360, 1.0f, 0.5f ) ));
        }
